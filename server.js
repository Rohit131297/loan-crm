...
CREATE TABLE IF NOT EXISTS leads (
 id TEXT PRIMARY KEY, customer TEXT NOT NULL, mobile TEXT NOT NULL, email TEXT DEFAULT '', banker TEXT DEFAULT '', connector TEXT DEFAULT '', bank TEXT DEFAULT '', bank_type TEXT DEFAULT 'Bank', product TEXT DEFAULT '', appno TEXT DEFAULT '', loan_account TEXT DEFAULT '', status TEXT NOT NULL DEFAULT 'New', loan_amount REAL NOT NULL DEFAULT 0, sanction_amount REAL NOT NULL DEFAULT 0, disb REAL NOT NULL DEFAULT 0, follow_date TEXT DEFAULT '', remark TEXT DEFAULT '', assigned_to TEXT DEFAULT '', created_at TEXT NOT NULL, updated_at TEXT NOT NULL
);
...
try { db.exec("ALTER TABLE leads ADD COLUMN product TEXT DEFAULT ''"); } catch {}
try { db.exec("ALTER TABLE leads ADD COLUMN bank_type TEXT DEFAULT 'Bank'"); } catch {}
const allowedStatuses=["New","Login","Sanction","Disbursed","Rejected"];
const allowedProducts=["Home Loan","Mortgage Loan","Personal Loan","Business Loan","Car Loan"];
const allowedBankTypes=["Bank","NBFC"];
...
app.post("/api/leads",auth,(req,res)=>{const b=req.body;if(!b.customer)return res.status(400).json({error:"Customer Name is required"});const id=String(b.id||crypto.randomUUID()),t=now(),assigned=req.user.role==="admin"?String(b.assigned_to||""):req.user.username;db.prepare(`INSERT INTO leads(id,customer,mobile,email,banker,connector,bank,bank_type,product,appno,loan_account,status,loan_amount,sanction_amount,disb,follow_date,remark,assigned_to,created_at,updated_at) VALUES(@id,@customer,@mobile,@email,@banker,@connector,@bank,@bank_type,@product,@appno,@loan_account,@status,@loan_amount,@sanction_amount,@disb,@follow_date,@remark,@assigned_to,@created_at,@updated_at)`).run({id,customer:String(b.customer),mobile:String(b.mobile||""),email:String(b.email||""),banker:String(b.banker||""),connector:String(b.connector||""),bank:String(b.bank||""),bank_type:allowedBankTypes.includes(b.bank_type)?b.bank_type:"Bank",product:allowedProducts.includes(b.product)?b.product:"",appno:String(b.appno||""),loan_account:String(b.loan_account||""),status:allowedStatuses.includes(b.status)?b.status:"New",loan_amount:Number(b.loan_amount||0),sanction_amount:Number(b.sanction_amount||0),disb:Number(b.disb||0),follow_date:String(b.follow_date||""),remark:String(b.remark||""),assigned_to:assigned,created_at:t,updated_at:t});res.json(db.prepare("SELECT * FROM leads WHERE id=?").get(id))});
app.put("/api/leads/:id",auth,(req,res)=>{const b=req.body,old=db.prepare(`SELECT * FROM leads WHERE id=?${leadScope(req)}`).get(req.params.id);if(!old)return res.status(404).json({error:"Lead not found"});const x={...old,...b,status:allowedStatuses.includes(b.status)?b.status:old.status,product:allowedProducts.includes(b.product)?b.product:old.product,bank_type:allowedBankTypes.includes(b.bank_type)?b.bank_type:old.bank_type,assigned_to:req.user.role==="admin"?String(b.assigned_to??old.assigned_to):old.assigned_to,updated_at:now()};db.prepare(`UPDATE leads SET customer=@customer,mobile=@mobile,email=@email,banker=@banker,connector=@connector,bank=@bank,bank_type=@bank_type,product=@product,appno=@appno,loan_account=@loan_account,status=@status,loan_amount=@loan_amount,sanction_amount=@sanction_amount,disb=@disb,follow_date=@follow_date,remark=@remark,assigned_to=@assigned_to,updated_at=@updated_at WHERE id=@id`).run(x);res.json(db.prepare("SELECT * FROM leads WHERE id=?").get(req.params.id))});
